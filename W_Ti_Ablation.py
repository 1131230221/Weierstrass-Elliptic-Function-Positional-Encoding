...
class WEFPositionalEncoding(nn.Module):
    ...
    def __init__(self, 
                 ...
                 use_4d: bool = True,
                 fix_pos_scale: bool = False,
                 device: torch.device = None):
        super().__init__()
        self.d_model = d_model
        self.max_h = max_h
        self.max_w = max_w
        self.use_4d = use_4d  # 保存 use_4d
        self.device = device if device is not None else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        # 根据 learnable_alpha 决定是参数还是固定值
        if learnable_alpha:
        ...
        # 根据 learnable_lattice 决定是参数还是固定值
        if learnable_lattice:
            self.alpha_learn = nn.Parameter(torch.tensor(0.12))
        else:
            self.register_buffer('alpha_learn', torch.tensor(0.12))
        ...
        # 添加一个可学习的缩放因子来控制位置编码的强度，或固定为1
        if fix_pos_scale:
            self.register_buffer('pos_scale', torch.ones(1))
        else:
...
class ImprovedViT_Ti(nn.Module):
    ...
    def __init__(self,
                 ...
                 wef_learnable_alpha: bool = True,
                 wef_learnable_lattice: bool = True,
                 wef_fix_pos_scale: bool = False,
                 ):
        super().__init__()
        ...
        if use_wef:
            self.pos_encoding = WEFPositionalEncoding(
                ...
                alpha_scale=0.15,
                learnable_alpha=wef_learnable_alpha,  # 传递可学习alpha的选项
                learnable_lattice=wef_learnable_lattice,  # 传递可学习晶格的选项
                use_4d=wef_use_4d,  # 传递4D/2D选项
                fix_pos_scale=wef_fix_pos_scale,  # 新增：固定位置编码强度
                )
        else:
            # 标准可学习位置编码
...
def create_cifar100_dataloaders(batch_size=160, num_workers=8, distributed=False, local_rank=0):
    """创建CIFAR-100数据加载器"""
    # 数据增强
    ...
    # 数据集
    data_root = '/root/shared-nvme/cifardata'
...
def train_model(model, train_loader, test_loader, num_epochs=120,  # 增加epoch数
                initial_lr=0.0015, warmup_epochs=15, use_amp=True, use_mixup=True, exp_name: str = 'default_exp',
                distributed=False, local_rank=0, world_size=1, train_sampler=None):
    """训练模型，使用改进的训练策略"""
    device = torch.device(f'cuda:{local_rank}' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    # 日志和模型保存目录
    save_dir = '/root/shared-nvme/Ablation_Experiment'
    ...
    best_acc = 0.0
    start_epoch = 0
    checkpoint_path = os.path.join(save_dir, f'checkpoint_{exp_name}.pth')  # 使用实验名
    log_path = os.path.join(save_dir, f'train_log_{exp_name}.txt')  # 使用实验名

    def is_main_process():
        return (not distributed) or (dist.get_rank() == 0)
    
    # 加载检查点
    if is_main_process() and os.path.exists(checkpoint_path):
    ...
    # 日志文件头
    if is_main_process() and start_epoch == 0 and (not os.path.exists(log_path) or os.path.getsize(log_path) == 0):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if use_mixup and random.random() > 0.4:  # 提高mixup概率
                ...
                if use_amp:
                    ...
                    scheduler.step()  # 每个batch更新学习率（放在optimizer.step()后）
                    scaler.update()
                else:
                    outputs = model(images)
                    loss = mixup_criterion(criterion, outputs, labels_a, labels_b, lam)
            ...
            # 每80个batch打印一次
            if is_main_process() and batch_idx % 80 == 0:
        ...
        if is_main_process() and test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), os.path.join(save_dir, f'best_wef_{exp_name}.pth'))
...
def main(args):  # 将 main 函数修改为接受一个参数对象
    """主函数"""
    # 分布式初始化
    distributed = False
    local_rank = 0
    ...
    if hasattr(args, 'local_rank'):
        ...
        print(f"RANK: {os.environ.get('RANK')}, LOCAL_RANK: {local_rank}, CUDA_VISIBLE_DEVICES: {os.environ.get('CUDA_VISIBLE_DEVICES')}")
    ...
    # 创建数据加载器，利用强大的硬件配置
    train_loader, test_loader, train_sampler = create_cifar100_dataloaders(batch_size=160, num_workers=8, distributed=distributed, local_rank=local_rank)
    ...
    model = ImprovedViT_Ti(
        ...
        wef_g2=args.g2,
        wef_g3=args.g3,
        wef_use_4d=args.use_4d,
        wef_learnable_alpha=not args.no_learnable_alpha,
        wef_learnable_lattice=not args.no_learnable_lattice,
        wef_fix_pos_scale=args.fix_pos_scale,
    )
    model.to(device)
    if distributed:
    ...
    if (not distributed) or (dist.get_rank() == 0):
        print(f"实验 '{args.exp_name}' 完成! 最高精度: {best_acc:.2f}%")
    if distributed:
        dist.destroy_process_group()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ViT-Ti with WEF Positional Encoding Ablation Study')
    parser.add_argument('--local_rank', type=int, default=0, help='DDP local rank')

    # 实验管理参数
    parser.add_argument('--exp_name', type=str, default=f'exp_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}',
                        help='为本次实验命名，用于区分日志和模型文件')

    # 实验1: 4D vs 2D
    parser.add_argument('--use_4d', action='store_true', default=True,
                        help='使用4D特征 (℘ 和 ℘′)。不设置此项则默认为True。')
    parser.add_argument('--no-use_4d', dest='use_4d', action='store_false',
                        help='不使用4D特征，仅使用2D (℘)。')

    # 实验2: 可学习参数
    parser.add_argument('--no-learnable-alpha', action='store_true',
                        help='设置此项后，tanh的缩放因子alpha_scale将不可学习')
    parser.add_argument('--no-learnable-lattice', action='store_true',
                        help='设置此项后，晶格形状参数alpha_learn将不可学习')

    # 实验3: 椭圆不变量
    parser.add_argument('--g2', type=float, default=1.0, help='椭圆不变量 g2')
    parser.add_argument('--g3', type=float, default=0.0, help='椭圆不变量 g3')

    # 实验4: 位置编码强度
    parser.add_argument('--fix-pos-scale', action='store_true', help='将位置编码强度pos_scale固定为1')
